...
function setup() {
  ...
  BrowserPrint.getDefaultDevice('printer', function (device) {
    ...
    var label = document.getElementById('labelPrintName');

    //console.log (document.getElementById ('labelPrintName'));
    if (label != null) {
      if (selected_device.name == undefined) {
        label.innerHTML = '<strong>Sin Impresora Asociada</strong>';
      } else {
        label.innerHTML = ' Impresora: ' + selected_device.name.substring(0, 11);
      }
    }
  }, function (error) {
    alert('No se ha detectado ninguna impresora!!\nInicie Browser Print de ZEBRA');//error);
...
var readCallback = function (readData) {
  if (readData === undefined || readData === null || readData === '') {
    alert('La impresora no esta conectada!');
  } else {
    alert(readData);
  }
}

async function readFromSelectedPrinter2() {
  return new Promise(function (resolve, reject) {
    checkPrinterStatus(function (text) {
      if (text == 'Ready to Print' || text == '') {
        resolve(text);
      } else {
        reject(text)
      }
...
async function checkPrinterStatus(finishedFunction) {
  return new Promise(function (resolve, reject) {
    selected_device.sendThenRead('~HQES',
      function (text) {
        var that = this;
